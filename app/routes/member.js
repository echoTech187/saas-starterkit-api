        ...
        db.execute(searchUserQuery, [email])
            .then(async ([rows]) => {
                if (rows.length > 0) {
                    ...
                    const codeVerify = randomInt(100000, 999999);
                    const codeExpired = new Date(Date.now() + 60 * 60 * 1000);
                    const query = 'INSERT INTO members (slug, username,fullname, email,image,provider_account_id,provider,provider_type, user_status, is_verified, otp_code, otp_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?,1, "N", ?, ?)';
                    db.execute(query, [slug, username, fullname, email, image, provider_account_id, provider, provider_type, codeVerify, codeExpired])
                        .then(async (result) => {
                            return res.status(200).json({
                                success: true,
                                message: 'Registrasi berhasil',
                            ...
                            if (err instanceof Error) {
                                return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 3', error: err.message });
                            } else {
                                return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 4', error: "Internal server error" });
                            }
                        });
                }
            }).catch((err) => {
                if (err instanceof Error) {
                    return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 5', error: err.message });
                } else {
                    return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 6', error: "Internal server error" });
                }
            });
    } catch (err) {
        if (err instanceof Error) {
            return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 7', error: err.message });
        } else {
            return res.status(500).json({ success: false, message: 'Terjadi kesalahan saat melakukan registrasi 8', error: "Internal server error" });
        }
    }
});
